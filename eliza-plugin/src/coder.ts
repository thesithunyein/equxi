...
export const EQUXI_PROGRAM_ID = new PublicKey(
...
export function pubkeyBytes(p: PublicKey): Buffer {
  return p.toBuffer();
}
...
/** `["agent", owner, name]` */
export function findAgentPDA(owner: PublicKey, name: string): PublicKey {
  return PublicKey.findProgramAddressSync(
    [Buffer.from("agent"), owner.toBuffer(), Buffer.from(name, "utf8")],
    EQUXI_PROGRAM_ID
  )[0];
}

/** `["bond", agent]` — one bond per agent. */
export function findBondPDA(agent: PublicKey): PublicKey {
  return PublicKey.findProgramAddressSync(
 ...
 */
export function findConstraintPDA(agent: PublicKey, index: number): PublicKey {
  return PublicKey.findProgramAddressSync(
    [Buffer.from("constraint"), agent.toBuffer(), u16le(index)],
    EQUXI_PROGRAM_ID
  )[0];
}

/** `["slash", agent, nonce]` where `nonce` is `config.total_slashed`. */
export function findSlashRecordPDA(
  agent: PublicKey,
  nonce: bigint | number
): PublicKey {
  return PublicKey.findProgramAddressSync(
    [Buffer.from("slash"), agent.toBuffer(), u64le(nonce)],
    EQUXI_PROGRAM_ID
  )[0];
}
...
export function registerAgentData(name: string, agentType: AgentType): Buffer {
...
export function createBondData(
...
/** `withdraw_bond()` */
export function withdrawBondData(): Buffer {
 ...
 */
export function encodeConstraintParams(params: ConstraintParamsInput): Buffer {
...
export function addConstraintData(
...
export function executeSlashData(
  reason: string,
...
function assertLength(data: Buffer, min: number, what: string): void {
  if (data.length < min) {
    throw new Error(
      `${what} account is ${data.length} bytes; expected at least ${min}`
    );
  }
}
...
export function decodeName(data: Buffer, offset = 40): string {
...
export function decodeAgent(data: Buffer): AgentAccount {
  ...
  return {
    ...
    agentType: data[72] as AgentType,
    trustScore: data[73],
    status: data[74] as AgentStatus,
 ...
 */
export function decodeBond(data: Buffer): BondAccount {
 ...
 */
export function decodeConstraint(data: Buffer): ConstraintAccount {
  assertLength(data, 339, "Constraint");
  return {
    agent: readPubkey(data, 8),
    constraintType: data[40] as ConstraintType,
    params: {
      ...
      allowedPrograms: Array.from({ length: ALLOWED_PROGRAMS_LEN }, (_, i) =>
        new PublicKey(data.subarray(73 + i * 32, 73 + (i + 1) * 32))
      ),
    },
 ...
 *                + bumped(1) = 259 bytes
 */
export function decodeSlashRecord(data: Buffer): SlashRecordAccount {
