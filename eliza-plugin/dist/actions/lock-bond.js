...
export const lockBondAction = {
    name: "EQUXI_LOCK_BOND",
    description: "Lock SOL as a safety bond for an agent on Equxi. Bond can be slashed if agent violates rules.",
    ...
    handler: async (runtime, message, _state, _options, callback) => {
        ...
        try {
            ...
            const amountSol = amountMatch
                ...
                : message.content?.amount || 0.5;
            const amountLamports = Math.floor(amountSol * LAMPORTS_PER_SOL);
            const agentName = message.content?.agentName ||
                textContent.match(/for (\w+)/i)?.[1] ||
                "Agent";
            const lockDuration = message.content?.lockDuration || 86400; // 1 day default
            ...
            const ix = await service.buildLockBond(operator, agentName, amountLamports, lockDuration);
...
//# sourceMappingURL=lock-bond.js.map